  ...
  return (
    <div className="items-page">
      ...
      <h1 className="page-title">
        ...
        <button className={`create-btn ${createOpened ? "creating" : ""}`} onClick={() => window.dispatchEvent(new Event("openCreate"))}>
          ...
          .map((dateKey) => (
            <div key={dateKey}>
              ...
              {grouped[dateKey].map((item) => (
                <div
                key={item.id}
                className={`item ${item.moving ? "item-move" : "animate-in"} ${item.deleting ? "deleting" : ""} ${isPastItem(item) ? "past" : ""}`}
              >                         
                  {/* Left */}
